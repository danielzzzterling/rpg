...
// abajo: codos flexionados (ang. codo < 90), muñecas al ras (misma altura)
const down = blank()
down[11] = { x: 10, y: 0 }; down[12] = { x: 12, y: 0 }
down[13] = { x: 10, y: 60 }; down[14] = { x: 12, y: 60 }
down[15] = { x: 40, y: 56 }; down[16] = { x: 42, y: 56 }
down[23] = { x: 22, y: 118 }; down[24] = { x: 28, y: 122 }
...
// arriba: brazos extendidos (colineales)
const up = blank()
up[11] = { x: 10, y: 0 }; up[12] = { x: 12, y: 0 }
up[13] = { x: 30, y: 90 }; up[14] = { x: 32, y: 90 }
up[15] = { x: 50, y: 180 }; up[16] = { x: 52, y: 180 }
up[23] = { x: 22, y: 118 }; up[24] = { x: 28, y: 122 }
...
// --- detector: conteo de reps ---
const c = new RepCounter()
assert.equal(c.update({ elbow: 175, wristGapNorm: 0 }), null, 'extender no cuenta')
assert.equal(c.update({ elbow: 60, wristGapNorm: 0 }), null, 'bajar no cuenta')
const r = c.update({ elbow: 175, wristGapNorm: 0 })
assert.ok(r && !r.oneHanded, 'rep normal')
assert.equal(c.count, 1)

const c2 = new RepCounter()
c2.update({ elbow: 175, wristGapNorm: 0 })
c2.update({ elbow: 60, wristGapNorm: 0.5 })
const r2 = c2.update({ elbow: 175, wristGapNorm: 0 })
...
c3.update({ elbow: 175, wristGapNorm: 0 })
c3.update({ elbow: 95, wristGapNorm: 0 })
c3.update({ elbow: 60, wristGapNorm: 0 })
c3.update({ elbow: 100, wristGapNorm: 0 })
c3.update({ elbow: 175, wristGapNorm: 0 })
